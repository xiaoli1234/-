...
public final class ExecuteAsyncTaskImpl implements IExecuteAsyncTask {
	...
	private ExecuteAsyncTaskImpl(Context context) {
		mContext	= context ;
		mQueue.clear() ;
	}
	
	/** default async execute */
	public static IExecuteAsyncTask defaultSyncExecutor(Context context){
		
		if(_instance == null){
			_instance = new ExecuteAsyncTaskImpl(context) ;
		}
	...
	public Worker executeTask(IAsyncTask task) {
		...
		if(!NetworkUtil.isNetworkConnected()){
			Toast.makeText(mContext, "网络连接不可用，处于离线状态！", Toast.LENGTH_SHORT).show();
			Log.e(TAG, "网络连接不可用，处于离线状态！") ;
			return null;
		}
	...
	public Worker executeTaskInNewThread(IAsyncTask task) {
		...
		if(!NetworkUtil.isNetworkConnected()){
			Toast.makeText(mContext, "网络连接不可用，处于离线状态！", Toast.LENGTH_LONG).show();
			Log.e(TAG, "网络连接不可用，处于离线状态！") ;
			return null;
		}
