...
public final class PropertySchema {
	...
	public String createTable(){
		...
		for (int index = 0, size = list.size(); index < size; index++) {
			...
			if (DateUtils.isEmpty(name) || DateUtils.isEmpty(tableColumn)) {
				continue;
			}
	...
	public PropertySQL insertObject(Object obj){
		...
		for(int index = 0, size = list.size(); index < size ; index++){
			...
			if (type == ColumnByte.class) {
				...
				param.put(name, Float.valueOf(Float.valueOf(value.toString()).floatValue()));
	...
	public PropertySQL updateObject(Object bean){
		...
		for(int index = 0 , size = propertys.size(); index < size ; index++){
			...
			if (type == ColumnShort.class) {
				content.put(name, Short.decode(value.toString()));
			} else if (type == ColumnInt.class) {
				content.put(name, Integer.decode(value.toString()));
			} else if (type == ColumnLong.class) {
				content.put(name, Long.decode(value.toString()));
			} else if (type == ColumnFloat.class) {
				content.put(name, Float.parseFloat(value.toString()));
			} else if (type == ColumnString.class) {
...
//				}
	...
	private PropertySQL queryObjectByPropertyField(List<PropertyField> list){
		...
		for(int index = 0 , size = list.size(); index < size ; index++){
			...
			if(DateUtils.isEmpty(name) || value == null){
				continue ;
			}
			
			if(index > 0 && type != ColumnBinary.class ){
			...
			if (type == ColumnByte.class) {
				...
				selection.append(" = ? ") ;
				params.add(Byte.toString(Integer.valueOf(value.toString()).byteValue()));
			} else if (type == ColumnShort.class) {
				selection.append(name) ;
				selection.append(" = ? ") ;
				params.add(Short.toString(Integer.valueOf(value.toString()).shortValue()));
			} else if (type == ColumnInt.class) {
				selection.append(name) ;
				selection.append(" = ? ") ;
				...
				selection.append(name) ;
				selection.append(" = ? ") ;
				params.add(Long.toString(Integer.valueOf(value.toString()).longValue()));
			} else if (type == ColumnFloat.class) {
				selection.append(name) ;
				selection.append(" = ? ") ;
	...
	public static PropertySchema create(Class<?> cls,boolean checkObject){
		...
		if(checkObject && DateUtils.isEmpty(table)){
			return null ;
		}
